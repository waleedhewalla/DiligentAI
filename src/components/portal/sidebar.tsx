...
import type { Accent } from "@/content/catalog/types";

/** Launchable offerings, passed from the server layout (built from the catalog). */
export type PortalProductLink = { slug: string; label: string; accent: Accent };
...
export function PortalNav({
  ...
  const items = [
    ...
    // Dynamic: one entry per offering that has a portal launch key.
    ...products.map((o) => ({
      href: `${base}/${o.slug}`,
      label: o.label,
      icon: <span className={cn("h-2.5 w-2.5 rounded-full", accentClasses[o.accent].bg)} />,
    })),
